...
use uaa::cli::{args::Cli, commands::*};
use uaa_core::{logging::logger, Result};
...
async fn main() -> Result<()> {
    ...
    let command_future = async {
        match cli.command {
            uaa::cli::args::Commands::CreateImage {
                arch,
                version,
                output,
                spec,
            ...
            uaa::cli::args::Commands::Cleanup {
                older_than_days,
                dry_run,
            } => cleanup_command(older_than_days, dry_run).await,
            uaa::cli::args::Commands::Install {
                remote,
                username,
                config,
                investigate_only,
            ...
            uaa::cli::args::Commands::SshInstall {
                host,
                hostname,
                username,
                config,
            ...
            uaa::cli::args::Commands::LocalInstall {
                hostname,
                investigate_only,
                dry_run,
                hold_on_failure,
            ...
            uaa::cli::args::Commands::Power {
                hostname,
                action,
                ipmi_password,
            } => power_command(&hostname, action, ipmi_password.as_deref()).await,
            uaa::cli::args::Commands::Enroll(args) => uaa::cli::enroll::enroll_command(args).await,
            uaa::cli::args::Commands::Luks(args) => uaa::cli::luks::luks_command(args).await,
            uaa::cli::args::Commands::Iso(args) => uaa::cli::iso::iso_command(args).await,
            uaa::cli::args::Commands::Config(args) => uaa::cli::config::config_command(args).await,
            uaa::cli::args::Commands::Image(args) => uaa::cli::image::image_command(args).await,
            uaa::cli::args::Commands::VmValidate(args) => {
                uaa::cli::vm_validate::vm_validate_command(args).await
            }
        }
    };
