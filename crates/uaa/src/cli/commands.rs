...
pub async fn validate_command(image_path: &str) -> Result<()> {
    ...
    if is_valid {
        ...
        return Err(uaa_core::error::AutoInstallError::ImageError(
...
pub async fn check_prerequisites_command() -> Result<()> {
    use uaa_core::utils::system::SystemUtils;
    ...
    if missing.is_empty() {
        ...
        Err(uaa_core::error::AutoInstallError::SystemError(format!(
...
pub async fn ssh_install_command(
    ...
    use uaa_core::network::ssh_installer::installer::PhaseSelection;
...
fn target_marker_present(root: &std::path::Path) -> bool {
    root.join(
        uaa_core::network::ssh_installer::installer::TARGET_MARKER_PATH.trim_start_matches('/'),
    )
    .exists()
}
...
fn create_local_installation_config(
    ...
    Ok(InstallationConfig {
        ...
        network_renderer: uaa_core::network::ssh_installer::config::default_network_renderer(),
...
fn detect_primary_disk(lsblk_json: &str) -> Result<String> {
    ...
    match chosen {
        ...
        None => Err(uaa_core::error::AutoInstallError::ValidationError(
...
pub async fn render_user_data_command(
    ...
    use uaa_core::autoinstall::{default_template, render_user_data, HostSpec};

    let template = match template_path {
        Some(path) => {
            std::fs::read_to_string(path).map_err(uaa_core::error::AutoInstallError::IoError)?
        }
    ...
    match output_path {
        Some(path) => {
            std::fs::write(path, &rendered).map_err(uaa_core::error::AutoInstallError::IoError)?;
...
pub async fn power_command(
    hostname: &str,
    action: uaa_core::power::PowerAction,
    ipmi_password: Option<&str>,
) -> Result<()> {
    use uaa_core::network::SshClient;
    use uaa_core::power::{run_power_action, validate_ipmi_request, POWER_SERVER};
