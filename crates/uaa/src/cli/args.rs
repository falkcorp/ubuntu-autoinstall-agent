// file: crates/uaa/src/cli/args.rs
// version: 2.8.1
...
pub enum Commands {
    ...
    /// Enroll a host into the fleet PKI (constellation — not yet implemented)
    Enroll(crate::cli::enroll::EnrollArgs),

    /// LUKS key management (constellation — not yet implemented)
    Luks(crate::cli::luks::LuksArgs),

    /// ISO remaster/build operations (constellation — not yet implemented)
    Iso(crate::cli::iso::IsoArgs),

    /// Config placement operations (constellation — not yet implemented)
    Config(crate::cli::config::ConfigArgs),

    /// Image build operations (constellation — not yet implemented)
    Image(crate::cli::image::ImageArgs),

    /// QEMU+swtpm VM validation (constellation — not yet implemented)
    VmValidate(crate::cli::vm_validate::VmValidateArgs),
}
